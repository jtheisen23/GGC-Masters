...
function PayoutsView({ evSort, setEvSort }) {
  ...
  return (
    React.createElement('div', null
      , React.createElement('div', { style: { background:"#162916", border:"1px solid #f5e642", borderRadius:10, padding:12, marginBottom:14 },}
        , React.createElement('h3', { style: { margin:"0 0 10px", color:"#f5e642", fontSize:13, fontWeight:700 },}, "TOTAL POT: $57,400"  )
        , React.createElement('div', { style: { display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 },}
          , [
            ...
            React.createElement('div', { key: label, style: { display:"flex", justifyContent:"space-between", background:"#0a160a", borderRadius:6, padding:"5px 9px", fontSize:11 },}
              , React.createElement('span', { style: { color:"#8fbc8f" },}, label)
              , React.createElement('span', { style: { color:"#f5e642", fontWeight:700 },}, "$", val.toLocaleString())
            )
          ))
        )
      )
      , React.createElement('div', { style: { display:"flex", gap:6, marginBottom:12, alignItems:"center", flexWrap:"wrap" },}
        , React.createElement('span', { style: { fontSize:11, color:"#6a9a6a" },}, "Sort by:" )
        , [["roi","ROI %"], ["ev","Expected Value"], ["sold","Amount Paid"]].map(([k, l]) => (
          React.createElement('button', { key: k, onClick: () => setEvSort(k), style: {
      ...
      , sortedEV.map((g) => {
        ...
        return (
          React.createElement('div', { key: g.name, style: { background:"#162916", border:"1px solid " + (roiPos ? "#2d6a2d" : "#2d4a2d"), borderRadius:10, padding:"12px 13px", marginBottom:8 },}
            , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 },}
              , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:8 },}
                , React.createElement('span', { style: { fontSize:13, fontWeight:800, background:"#0a160a", borderRadius:6, padding:"2px 7px", color:"#f5e642" },}, ico)
                , React.createElement('div', null
                  , React.createElement('div', { style: { fontWeight:800, fontSize:14 },}, g.name)
                  , React.createElement('div', { style: { fontSize:10, color:"#6a9a6a" },}
                    , "Paid: $" + g.sold.toLocaleString() + " | EV: $" + g.ev.toLocaleString()
                  )
                )
              )
              , React.createElement('div', { style: { textAlign:"right" },}
                , React.createElement('div', { style: { fontSize:22, fontWeight:900, color:roiColor, lineHeight:1 },}, (g.roi > 0 ? "+" : "") + g.roi + "%")
                , React.createElement('div', { style: { fontSize:10, color:"#6a9a6a" },}, "expected ROI" )
              )
            )
            , React.createElement('div', { style: { display:"grid", gridTemplateColumns:"repeat(6,1fr)", gap:4, marginBottom:6 },}
              , [["Finish",g.finish,"#4ade80"],["Daily",g.daily,"#c084fc"],["R2",g.r2,"#60a5fa"],["R3",g.r3,"#60a5fa"],["HIO",g.hio,"#fb923c"],["Par3",g.par3,"#f5e642"]].map(([label, val, color]) => (
                React.createElement('div', { key: label, style: { background:"#0a160a", borderRadius:6, padding:"4px 5px", textAlign:"center" },}
                  , React.createElement('div', { style: { fontSize:11, fontWeight:700, color },}, "$" + val.toLocaleString())
                  , React.createElement('div', { style: { fontSize:8, color:"#6a9a6a" },}, label)
                )
              ))
            )
            , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:8, fontSize:10, color:"#6a9a6a" },}
              , React.createElement('span', null, "P+L: " + (g.ev >= g.sold ? "+" : "") + (g.ev - g.sold).toLocaleString())
              , React.createElement('div', { style: { flex:1, background:"#0a160a", borderRadius:3, height:5, overflow:"hidden" },}
                , React.createElement('div', { style: { width: Math.min(100,(g.ev/g.sold)*100) + "%", height:"100%", background:roiColor, borderRadius:3 },} )
              )
              , React.createElement('span', null, Math.round((g.ev/g.sold)*100) + "c per $1")
            )
          )
        );
      })
...
function PredictView() {
  ...
  return (
    React.createElement('div', null
      , React.createElement('div', { style: { background:"#162916", border:"1px solid #f5e642", borderRadius:10, padding:12, marginBottom:14, fontSize:12, color:"#8fbc8f" },}
        , React.createElement('b', { style: { color:"#f5e642" },}, "Regression model from 9 actual sales."     )
        , " Formula: Price ~ $118 x Win% + $2,042. Avg sold: $" + avgSold.toLocaleString() + "."
      )
      , React.createElement('div', { style: { background:"#0d1f0d", border:"1px solid #2d4a2d", borderRadius:10, padding:12, marginBottom:14 },}
        , React.createElement('h3', { style: { margin:"0 0 10px", color:"#6a9a6a", fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:0.5 },}, "Sold Groups - Value Check"    )
        , soldGroups.map(g => {
          const fair = _optionalChain([enrichedGroups, 'access', _ => _.find, 'call', _2 => _2(eg => eg.name === g.name), 'optionalAccess', _3 => _3.value]) || 0;
          const diff = g.sold - fair;
          const isSteal = diff < -400;
          const isOverpay = diff > 400;
          return (
            React.createElement('div', { key: g.name, style: { display:"flex", alignItems:"center", gap:8, marginBottom:6, flexWrap:"wrap" },}
              , React.createElement('span', { style: { fontSize:12, fontWeight:800, background:"#162916", borderRadius:4, padding:"1px 6px", color:"#f5e642" },}, g.icon)
              , React.createElement('span', { style: { width:120, fontSize:12, color:"#c8e6c9", fontWeight:600 },}, g.name)
              , React.createElement('span', { style: { fontSize:12, color:"#f5e642", fontWeight:700, minWidth:50 },}, "$" + g.sold.toLocaleString())
              , React.createElement('span', { style: { fontSize:10, color:"#6a9a6a" },}, "fair ~$" + fair.toLocaleString())
              , React.createElement('span', { style: { marginLeft:"auto", fontSize:10, fontWeight:700, padding:"2px 7px", borderRadius:8,
      ...
      , React.createElement('h3', { style: { color:"#f5e642", fontSize:13, fontWeight:700, margin:"0 0 10px" },}, "All Predicted Prices vs Actuals"    )
      , predictions.map((p) => {
        const onModel = p.actual <= p.high && p.actual >= p.low;
        return (
          React.createElement('div', { key: p.name, style: { background:"#162916", border:"1px solid #2d4a2d", borderRadius:10, padding:"12px 14px", marginBottom:8 },}
            , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:6 },}
              , React.createElement('div', null
                , React.createElement('div', { style: { fontWeight:800, fontSize:14, color:"#e8f5e9" },}, p.name)
                , React.createElement('div', { style: { fontSize:10, color:"#6a9a6a" },}, "Model: $" + p.pred.toLocaleString() + " | Range: $" + p.low.toLocaleString() + "-$" + p.high.toLocaleString())
              )
              , React.createElement('span', { style: { fontSize:11, padding:"3px 8px", borderRadius:8, fontWeight:700,
            ...
            , React.createElement('div', { style: { position:"relative", background:"#0a160a", borderRadius:4, height:8, marginBottom:6 },}
              , React.createElement('div', { style: { position:"absolute", left:(p.low/barMax*100)+"%", width:((p.high-p.low)/barMax*100)+"%", height:"100%", background:"#2d4a2d", borderRadius:4 },} )
              , React.createElement('div', { style: { position:"absolute", left:(p.pred/barMax*100)+"%", transform:"translateX(-50%)", width:3, height:"100%", background:"#f5e642", borderRadius:2 },} )
              , React.createElement('div', { style: { position:"absolute", left:(p.actual/barMax*100)+"%", transform:"translateX(-50%)", width:3, height:"100%", background:"#f87171", borderRadius:2 },} )
            )
            , React.createElement('div', { style: { fontSize:11, color:"#8fbc8f", fontStyle:"italic" },}, p.tip)
          )
        );
      })
    )
 ...
 function AuctionApp() {
  const [view, setView] = React.useState.call(void 0, "board");
  const [selected, setSelected] = React.useState.call(void 0, null);
  const [sortBy, setSortBy] = React.useState.call(void 0, "winProb");
  const [evSort, setEvSort] = React.useState.call(void 0, "roi");
  ...
  return (
    React.createElement('div', { style: { fontFamily:"'Segoe UI', system-ui, sans-serif", background:"#0a160a", minHeight:"100vh", color:"#e8f5e9" },}
      , React.createElement('div', { style: { background:"linear-gradient(135deg, #162916, #0d2010, #162916)", borderBottom:"2px solid #f5e642", padding:"16px 16px 12px" },}
        , React.createElement('div', { style: { maxWidth:860, margin:"0 auto" },}
          , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:14, marginBottom:4 },}
            ...
            , React.createElement('div', null
              ...
              , React.createElement('h1', { style: { margin:0, fontSize:20, fontWeight:900, color:"#f5e642", letterSpacing:1 },}, "2026 GGC MASTERS AUCTION"   )
              , React.createElement('p', { style: { margin:0, fontSize:10, color:"#6a9a6a", letterSpacing:2 },}, "18 GROUPS | APR 9-12 | TOTAL POT $57,400"        )
            )
          )
      ...
      , React.createElement('div', { style: { maxWidth:860, margin:"0 auto", padding:"14px 12px 48px" },}

        , view === "board" && (
          React.createElement('div', null
            , React.createElement('div', { style: { background:"#162916", border:"1px solid #f5e642", borderRadius:10, padding:12, marginBottom:14, fontSize:12, color:"#8fbc8f" },}
              , React.createElement('b', { style: { color:"#f5e642" },}, "Values calibrated from 9 actual sales"     )
              , " (median ~$337/win%). "
              , React.createElement('b', { style: { color:"#4ade80" },}, "Scheffler+ was the steal"   )
              , " at $4,100 (fair ~$6,300). All 18 groups sold. Total pot: "
              , React.createElement('b', { style: { color:"#f5e642" },}, "$57,400")
            )
            , React.createElement('div', { style: { display:"flex", gap:6, marginBottom:12, flexWrap:"wrap", alignItems:"center" },}
              ...
              , [["winProb","Win%"],["value","$ Value"],["r1","R1 Lead"],["r3","R3 Lead"]].map(([k, l]) => (
                React.createElement('button', { key: k, onClick: () => setSortBy(k), style: {
                  padding:"4px 10px", borderRadius:12, border:"1px solid #2d4a2d", cursor:"pointer", fontSize:11,
              ...
              return (
                React.createElement('div', { key: g.name,
                  ...
                  , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:10 },}
                    , React.createElement('div', { style: { width:40, flexShrink:0, textAlign:"center" },}
                      , React.createElement('div', { style: { fontSize:13, fontWeight:800, background:"#0a160a", borderRadius:6, padding:"2px 4px", color:"#f5e642" },}, g.icon)
                      , React.createElement('div', { style: { fontSize:9, color:"#6a9a6a" },}, "#" + (i+1))
                    )
                    , React.createElement('div', { style: { flex:1, minWidth:0 },}
                      , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3 },}
                        , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:7, flexWrap:"wrap" },}
                          , React.createElement('span', { style: { fontWeight:800, fontSize:14 },}, g.name)
                          , React.createElement('span', { style: { fontSize:9, padding:"2px 7px", borderRadius:8, background:tier.bg, color:tier.color, fontWeight:700 },}, tier.label)
                          , React.createElement('span', { style: { fontSize:9, padding:"2px 7px", borderRadius:8, background:"#7f1d1d", color:"#fca5a5", fontWeight:700 },}, "SOLD $" + g.sold.toLocaleString())
                          , g.owner ? React.createElement('span', { style: { fontSize:9, padding:"2px 7px", borderRadius:8, background:"#1e3a5f", color:"#93c5fd", fontWeight:700 },}, g.owner) : null
                        )
                        , React.createElement('div', { style: { fontSize:13, fontWeight:900, color:"#f87171", lineHeight:1, marginLeft:8 },}, "$" + g.sold.toLocaleString())
                      )
                      , React.createElement('div', { style: { background:"#0a160a", borderRadius:4, height:6, overflow:"hidden", marginBottom:4 },}
                        , React.createElement('div', { style: { width:pct+"%", height:"100%", borderRadius:4, background:"#6b7280" },} )
                      )
                      , React.createElement('div', { style: { display:"flex", gap:12, fontSize:10, color:"#6a9a6a", flexWrap:"wrap" },}
                        , React.createElement('span', null, "Win: ", React.createElement('b', { style: { color:"#a5d6a7" },}, g.winProb + "%"))
                        , React.createElement('span', null, "R1: ", React.createElement('b', { style: { color:"#c084fc" },}, g.r1 + "%"))
                        , React.createElement('span', null, "R3: ", React.createElement('b', { style: { color:"#60a5fa" },}, g.r3 + "%"))
                        , React.createElement('span', { style: { marginLeft:"auto", color:"#4a7a4a", fontSize:9 },}, "* " + (star ? star.name : "") + " (+" + (star ? star.odds.toLocaleString() : "") + ")")
                      )
                    )
                  )
                )
          ...
          React.createElement('div', null
            , (() => {
              ...
              return ownerList.map(([owner, ownerGroups]) => {
                ...
                const totalEV = ownerGroups.reduce((s, g) => s + (_optionalChain([evByGroup, 'access', _4 => _4[g.name], 'optionalAccess', _5 => _5.ev]) || 0), 0);
                ...
                return (
                  React.createElement('div', { key: owner, style: { background:"#162916", border:"1px solid #2d4a2d", borderRadius:12, padding:14, marginBottom:12 },}
                    , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 },}
                      , React.createElement('div', null
                        , React.createElement('h3', { style: { margin:0, fontSize:16, fontWeight:900, color:"#f5e642" },}, owner)
                        , React.createElement('div', { style: { fontSize:11, color:"#6a9a6a", marginTop:2 },}, ownerGroups.length + " group" + (ownerGroups.length > 1 ? "s" : ""))
                      )
                      , React.createElement('div', { style: { textAlign:"right" },}
                        , React.createElement('div', { style: { fontSize:20, fontWeight:900, color:"#f87171" },}, "$" + totalSpent.toLocaleString())
                        , React.createElement('div', { style: { fontSize:10, color:"#6a9a6a" },}, "total spent" )
                      )
                    )
                    , React.createElement('div', { style: { display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:10 },}
                      , React.createElement('div', { style: { background:"#0a160a", borderRadius:8, padding:"8px 10px", textAlign:"center" },}
                        , React.createElement('div', { style: { fontSize:18, fontWeight:900, color:"#4ade80" },}, Math.round(totalWin * 10) / 10 + "%")
                        , React.createElement('div', { style: { fontSize:9, color:"#6a9a6a", textTransform:"uppercase" },}, "Win Prob" )
                      )
                      , React.createElement('div', { style: { background:"#0a160a", borderRadius:8, padding:"8px 10px", textAlign:"center" },}
                        , React.createElement('div', { style: { fontSize:18, fontWeight:900, color:"#60a5fa" },}, "$" + Math.round(totalEV).toLocaleString())
                        , React.createElement('div', { style: { fontSize:9, color:"#6a9a6a", textTransform:"uppercase" },}, "Expected Value" )
                      )
                      , React.createElement('div', { style: { background:"#0a160a", borderRadius:8, padding:"8px 10px", textAlign:"center" },}
                        , React.createElement('div', { style: { fontSize:18, fontWeight:900, color:roiColor },}, (roi > 0 ? "+" : "") + roi + "%")
                        , React.createElement('div', { style: { fontSize:9, color:"#6a9a6a", textTransform:"uppercase" },}, "Expected ROI" )
                      )
                    )
                    , ownerGroups.map(g => {
                      ...
                      const tier = tierFor(g.winProb);
                      return (
                        React.createElement('div', { key: g.name, style: { display:"flex", alignItems:"center", gap:8, padding:"7px 10px", background:"#0a160a", borderRadius:8, marginBottom:5 },}
                          , React.createElement('span', { style: { fontSize:11, fontWeight:800, color:"#f5e642", minWidth:32 },}, g.icon)
                          , React.createElement('span', { style: { fontWeight:700, fontSize:13, flex:1 },}, g.name)
                          , React.createElement('span', { style: { fontSize:9, padding:"2px 6px", borderRadius:6, background:tier.bg, color:tier.color, fontWeight:700 },}, tier.label)
                          , React.createElement('span', { style: { fontSize:11, color:"#f87171", fontWeight:700, minWidth:55, textAlign:"right" },}, "$" + g.sold.toLocaleString())
                          , React.createElement('span', { style: { fontSize:10, color:"#6a9a6a", minWidth:65, textAlign:"right" },}, "Win: " + g.winProb + "%")
                          , ev ? React.createElement('span', { style: { fontSize:10, minWidth:65, textAlign:"right", color: ev.roi >= 0 ? "#4ade80" : "#f87171", fontWeight:700 },}, (ev.roi > 0 ? "+" : "") + ev.roi + "% ROI") : null
                        )
                      );
                    })
                  )
        ...
        , view === "payouts" && React.createElement(PayoutsView, { evSort: evSort, setEvSort: setEvSort,} )

        , view === "predict" && React.createElement(PredictView, null )

        , view === "rounds" && (
          React.createElement('div', null
            , React.createElement('div', { style: { background:"#162916", border:"1px solid #2d6a2d", borderRadius:10, padding:12, marginBottom:14, fontSize:12, color:"#8fbc8f" },}, "Probability that the group holds the lead after each round. 54-hole (R3) leaders win ~50% of the time at Augusta. R1 leaders convert at ~14%."
            ...
            , [["r1","R1 - Thursday","#c084fc"],["r2","R2 - Friday","#60a5fa"],["r3","R3 - Saturday","#4ade80"],["winProb","Winner","#f5e642"]].map(([key, label, color]) => {
              ...
              const mx = s[0][key];
              return (
                React.createElement('div', { key: key, style: { background:"#162916", border:"1px solid #2d4a2d", borderRadius:10, padding:13, marginBottom:12 },}
                  , React.createElement('h3', { style: { margin:"0 0 10px", color, fontSize:13, fontWeight:700 },}, label)
                  , s.map((g, i) => (
                    React.createElement('div', { key: g.name, style: { display:"flex", alignItems:"center", gap:8, marginBottom:5 },}
                      , React.createElement('span', { style: { width:18, fontSize:10, color:"#6a9a6a", textAlign:"right" },}, i+1)
                      , React.createElement('span', { style: { width:140, fontSize:12, fontWeight:i===0?700:400, color:i===0?"#f5e642":"#c8e6c9", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" },}, g.name)
                      , React.createElement('div', { style: { flex:1, background:"#0a160a", borderRadius:4, height:8, overflow:"hidden" },}
                        , React.createElement('div', { style: { width:(g[key]/mx*100)+"%", height:"100%", background:color, borderRadius:4 },} )
                      )
                      , React.createElement('span', { style: { width:38, textAlign:"right", fontSize:12, fontWeight:700, color:"#a5d6a7" },}, g[key] + "%")
                    )
                  ))
                )
              );
          ...
          React.createElement('div', null
            , React.createElement('div', { style: { display:"flex", gap:6, marginBottom:12, flexWrap:"wrap" },}
              , enrichedGroups.map(g => (
                React.createElement('button', { key: g.name, onClick: () => setSelected(g.name), style: {
            ...
            , !selGroup && (
              React.createElement('div', { style: { textAlign:"center", color:"#4a7a4a", padding:40, fontSize:14 },}, "Select a group above"   )
            )

            , selGroup && (
              React.createElement('div', null
                , React.createElement('div', { style: { background:"#162916", border:"2px solid #2d6a2d", borderRadius:12, padding:16, marginBottom:12 },}
                  , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 },}
                    , React.createElement('div', null
                      , React.createElement('h2', { style: { margin:0, fontSize:20, color:"#f5e642" },}, selGroup.name)
                      , React.createElement('div', { style: { fontSize:11, color:"#6a9a6a", marginTop:2 },}, selGroup.members.length + " players")
                      , selGroup.owner ? React.createElement('div', { style: { fontSize:12, color:"#93c5fd", fontWeight:700, marginTop:4 },}, selGroup.owner) : null
                    )
                    , React.createElement('div', { style: { textAlign:"center" },}
                      , React.createElement('div', { style: { fontSize:32, fontWeight:900, color:"#f87171", lineHeight:1 },}, "$" + selGroup.sold.toLocaleString())
                      , React.createElement('div', { style: { fontSize:9, color:"#6a9a6a", textTransform:"uppercase" },}, "Sold Price" )
                    )
                  )
                  , React.createElement('div', { style: { display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:8 },}
                    , [["Win%", selGroup.winProb+"%", "#4ade80"],["R1 Lead", selGroup.r1+"%", "#c084fc"],["R2 Lead", selGroup.r2+"%", "#60a5fa"],["R3 Lead", selGroup.r3+"%", "#fb923c"]].map(([l, v, c]) => (
                      React.createElement('div', { key: l, style: { background:"#0a160a", borderRadius:8, padding:"8px 10px", textAlign:"center" },}
                        , React.createElement('div', { style: { fontSize:20, fontWeight:900, color:c },}, v)
                        , React.createElement('div', { style: { fontSize:9, color:"#6a9a6a", textTransform:"uppercase" },}, l)
                      )
                    ))
                  )
                )
                , React.createElement('h3', { style: { color:"#6a9a6a", fontSize:11, fontWeight:700, margin:"0 0 8px", textTransform:"uppercase", letterSpacing:0.5 },}, "Players (by win probability)"   )
                , selGroup.playerList.map((p, i) => (
                  React.createElement('div', { key: p.name, style: { background:"#162916", border:"1px solid #2d4a2d", borderRadius:9, padding:"10px 12px", marginBottom:6 },}
                    , React.createElement('div', { style: { display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 },}
                      , React.createElement('div', { style: { display:"flex", alignItems:"center", gap:8 },}
                        , React.createElement('span', { style: { fontSize:10, color:"#6a9a6a" },}, "#" + (i+1))
                        , React.createElement('span', { style: { fontWeight:700, fontSize:13, color: i===0 ? "#f5e642" : "#e8f5e9" },}, p.name)
                      )
                      , React.createElement('div', { style: { display:"flex", gap:10, fontSize:11 },}
                        , React.createElement('span', { style: { color:"#4ade80" },}, "Win: ", React.createElement('b', null, p.winProb + "%"))
                        , React.createElement('span', { style: { color:"#f5e642" },}, "+" + p.odds.toLocaleString())
                      )
                    )
                    , React.createElement('div', { style: { display:"flex", gap:12, fontSize:10, color:"#6a9a6a", marginBottom:4 },}
                      , React.createElement('span', null, "R1: ", React.createElement('b', { style: { color:"#c084fc" },}, p.r1 + "%"))
                      , React.createElement('span', null, "R2: ", React.createElement('b', { style: { color:"#60a5fa" },}, p.r2 + "%"))
                      , React.createElement('span', null, "R3: ", React.createElement('b', { style: { color:"#fb923c" },}, p.r3 + "%"))
                    )
                    , React.createElement('div', { style: { fontSize:11, color:"#6a9a6a", fontStyle:"italic" },}, p.notes)
                  )
                ))
              )
            )
      ...
      , React.createElement('div', { style: { textAlign:"center", padding:12, borderTop:"1px solid #162916", fontSize:10, color:"#3a5a3a" },}, "Odds from DraftKings/FanDuel as of Apr 6, 2026 | For entertainment purposes only"
